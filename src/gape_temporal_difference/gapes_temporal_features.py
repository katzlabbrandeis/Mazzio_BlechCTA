...
for directory in [data_dir, plot_dir, artifacts_dir]:
    if not os.path.exists(directory):
        print(f"Directory {directory} does not exist. Please create it and add the necessary files.")
...
##############################
# Test for significance of regression of features on time using a simple linear regression for each feature
p_values = []
for i in range(cut_X.shape[1]):
    slope, intercept, r_value, p_value, std_err = linregress(cut_y, cut_X[:, i])
    p_values.append(p_value)

# Write out as a csv
p_values_df = pd.DataFrame({
    'feature_index': np.arange(cut_X.shape[1]),
    'p_value': p_values
    })
p_values_df.to_csv(os.path.join(artifacts_dir, 'feature_time_regression_p_values.csv'), index=False)
...
# Get loadings of the first 3 PCs
loadings = pca.components_.T
# feature_sum = np.sum(np.abs(loadings), axis=1)
# Plot loadings as a heatmap
fig, ax = plt.subplots(figsize=(5, 10))
im = ax.imshow(np.abs(loadings), aspect='auto', cmap='viridis')
ax.set_xlabel('Principal Component')
ax.set_ylabel('Original Feature Index')
ax.set_title('Absolute PCA Loadings (Cutoff at 200ms)')
plt.colorbar(im, label='Loading Value')
fig.savefig(os.path.join(plot_dir, 'pca_loadings_cutoff_200ms.png'), dpi=300)
plt.close()

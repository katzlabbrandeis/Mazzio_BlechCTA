...
df = df[["features", "segment_raw", "segment_norm_interp", 
         ...
         "raw_features", "event_type", "session_ind", "exp_day_type", "exp_day_num", "licl_conc", "num_of_cta"]] # Keep only needed columns
...
mask = df["features"].apply(lambda x: tuple(x)).duplicated()
df_dupes = df[~mask]
df = df_dupes
# ==============================================================================
# Save new dataframe
...
output_file_path = os.path.join(dirname, 'all_NaCl_datasets_emg_pred.pkl')
